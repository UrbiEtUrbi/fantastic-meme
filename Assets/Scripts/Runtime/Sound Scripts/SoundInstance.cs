...
public class SoundInstance : PoolObject
{
    ...
    public void Set(SoundItem item, Sound sound, Transform target, float minDistance, float maxDistance, bool spatialBlend)
    {
       ...
       var t = 0f;
        var blend = 1f;
        if (followTarget) {
            t = Vector3.Distance(ControllerGame.Player.transform.position, target.position)/m_AudioSource.maxDistance;
    ...
    private void Update()
    {
        ...
        if (m_AudioSource.isPlaying)
        {
            ...
            if (followTarget && m_SpatialBlend)
            {
                ...
                var t = Vector3.Distance(ControllerGame.Player.transform.position, m_Target.position) / m_AudioSource.maxDistance;
